module.exports=async function({bob,alice,actor,check,go,login,contains,shot,assert,tag,API,BASE,OUT,fs,path}) {
  ...
  await check('Doppelte Registrierung zeigt verständliche Fehlermeldung',async()=>{await fillRegistration(tag);await contains(fresh,'.message.error','bereits vergeben');});
