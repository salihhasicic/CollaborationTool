...
async function main() {
  ...
  if(phase==='pages-chat') {
    const routes=['/start','/projects','/projects/new','/tasks/new','/users','/profile/2','/profile/1','/team/1','/team_manage','/teams/new','/chat/1','/chats','/private_chat/1','/register','/login'];
    const inventory=[];
    ...
    await check('Teamchat: KI-Fehler wird verständlich angezeigt',async()=>{await bob.locator('#ai-input').fill('Antworte mit einem kurzen Hallo.');await bob.locator('.ai-chat-row button').click();await bob.waitForFunction(()=>!['Noch keine Frage gestellt.','KI denkt ...'].includes(document.querySelector('#ai-answer').innerText));const text=await bob.locator('#ai-answer').innerText();assert.ok(text.includes('nicht eingerichtet') || !text.includes('Fehler'),text);return text;});
    await check('Teamchat: keine Übernahme einer KI-Fehlermeldung',async()=>{await bob.getByRole('button',{name:'AI-Antwort vorschlagen'}).click();await bob.waitForFunction(()=>!document.querySelector('#ai-suggestion').innerText.includes('denkt')); if ((await bob.locator('#ai-suggestion').innerText()).includes('nicht eingerichtet')) {assert.equal(await bob.locator('.ai-suggestion-action').count(),0);return 'Nicht konfiguriert: klare Meldung, kein fehlerhafter Vorschlag';}const text=await bob.locator('#ai-suggestion').innerText();await bob.locator('.ai-suggestion-action').click();const value=await bob.locator('#message').inputValue();assert.ok(value.length>0);assert.ok(!value.includes('Fehler'),text);});
    await check('Chat: Zeitangabe entspricht Europe/Zurich',async()=>{const item=bob.locator('.chat-message').filter({hasText:tm2});const raw=await item.locator('[data-timestamp]').getAttribute('data-timestamp');const displayed=await item.locator('.chat-timestamp').innerText();const expected=await bob.evaluate(raw=>new Date(raw).toLocaleString('de-DE'),raw);assert.equal(displayed,expected,JSON.stringify({raw,displayed,expected}));});
  }
